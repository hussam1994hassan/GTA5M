...
const RegisterPage = () => {
  ...
  const handleSubmit = async () => {
    ...
    // Show success notification (you would use react-hot-toast here)
...
const toggleMode = () => {
  if (isLogin) {
    // لو هو في login وروحنا لـ register
    navigate('/register', { replace: true });

  } else {
    // لو هو في register وروحنا لـ login
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white overflow-hidden relative flex items-center justify-center">
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-1/4 right-1/6 w-96 h-96 bg-cyan-500/8 rounded-full blur-3xl animate-pulse" style={{animationDelay: '2s'}}></div>
        <div className="absolute top-3/4 left-3/4 w-64 h-64 bg-blue-500/10 rounded-full blur-3xl animate-pulse" style={{animationDelay: '4s'}}></div>

        {/* Code Elements */}
        <div className="absolute inset-0 font-mono text-xs opacity-5">
          {['{ auth }', '[ secure ]', '< vault >', '( login )', '// protected', 'const user', 'return token', 'async verify'].map((symbol, i) => (
            <div 
              key={i}
              className={`absolute ${i % 3 === 0 ? 'text-emerald-400' : i % 3 === 1 ? 'text-cyan-400' : 'text-blue-400'}`}
              style={{
          ...
          <div className="hidden lg:block space-y-8">
            ...
            <div className="flex items-center space-x-4 group">
              ...
              <div>
                ...
                <p className="text-emerald-400 font-mono text-lg">SECURE_ACCESS_PORTAL</p>
              </div>
            </div>
            ...
            <div className="space-y-6">
              <h2 className="text-3xl font-bold text-white mb-8">
                {isLogin ? 'Welcome Back, Agent' : 'Join the Vault Network'}
              </h2>
              
              <div className="space-y-4">
                {[
                  { icon: <Fingerprint className="w-6 h-6" />, title: "Biometric Security", desc: "Multi-layer authentication protocol" },
                  { icon: <Database className="w-6 h-6" />, title: "Encrypted Vault", desc: "Military-grade data protection" },
                  { icon: <Globe className="w-6 h-6" />, title: "Global Access", desc: "Secure connection worldwide" },
                  { icon: <Smartphone className="w-6 h-6" />, title: "2FA Protection", desc: "Enhanced security verification" }
                ].map((feature, index) => (
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="bg-gradient-to-br from-gray-800/60 to-gray-900/60 p-6 rounded-xl border border-emerald-400/20 backdrop-blur-sm">
                <div className="text-2xl font-black text-emerald-400 mb-2">99.9%</div>
                <div className="text-gray-400 font-mono text-sm">Security Uptime</div>
              </div>
              <div className="bg-gradient-to-br from-gray-800/60 to-gray-900/60 p-6 rounded-xl border border-cyan-400/20 backdrop-blur-sm">
                <div className="text-2xl font-black text-cyan-400 mb-2">256bit</div>
                <div className="text-gray-400 font-mono text-sm">Encryption Level</div>
              </div>
            </div>
          </div>
            ...
            <div className="bg-gray-800/80 backdrop-blur-sm border border-cyan-500/30 rounded-t-2xl p-4">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="text-gray-400 font-mono ml-4 text-sm">
                  {isLogin ? 'vault_login.exe' : 'vault_register.exe'}
                </span>
              </div>
              <div className="text-emerald-400 font-mono text-sm">
                $ {isLogin ? 'authenticate --secure' : 'register --vault'} 
            ...
            <div className="bg-gradient-to-br from-gray-800/90 to-gray-900/90 backdrop-blur-xl border-x border-b border-cyan-500/30 rounded-b-2xl p-8 shadow-2xl">
              ...
              <div className="flex mb-8 bg-gray-800/50 rounded-xl p-1">
                <button
                  ...
                  className={`flex-1 py-3 px-4 rounded-lg font-mono font-semibold transition-all duration-300 ${
                    isLogin 
                  ...
                  <span className="relative z-10 flex items-center justify-center gap-3">
                    {isLoading ? (
                      <>
                        ...
                        {isLogin ? 'AUTHENTICATING...' : 'CREATING_VAULT...'}
                      </>
                    ) : (
                      <>
                        <Shield className="w-6 h-6" />
                        {isLogin ? '[AUTHENTICATE]' : '[CREATE_VAULT]'}
                ...
                <div className="space-y-4">
                  ...
                  <div className="text-center">
                    <span className="text-gray-400 font-mono text-sm">
                      {isLogin ? "Don't have vault access? " : "Already have an account? "}
                      <button
                        type="button"
                        onClick={toggleMode}
                        className="text-emerald-400 hover:text-cyan-400 font-bold transition-colors"
                      >
                        {isLogin ? '[CREATE_VAULT]' : '[LOGIN]'}
                      </button>
                    </span>
                  </div>
                </div>
              ...
              <div className="mt-8 p-4 bg-gradient-to-r from-emerald-500/10 to-cyan-500/10 border border-emerald-400/20 rounded-xl">
                <div className="flex items-center gap-3">
                  ...
                  <p className="text-gray-300 text-sm font-mono">
                    Your data is protected by military-grade encryption and stored in secure vaults.
                  </p>
                </div>
              </div>
            </div>
